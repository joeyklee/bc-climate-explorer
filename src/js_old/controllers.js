    ...
    */
    ...
    async function loadClimateNormalData() {
        ...
        let data = await $.getJSON(encodeURI(query), () => {
            el.x.scatterplot.data = data.rows.map(obj => {
                return obj[xSelection]
            });
            el.x.scatterplot.zones = data.rows.map(obj => {
                return obj.map_label
            });

            el.y.scatterplot.data = data.rows.map(obj => {
                return obj[ySelection]
            });
            el.y.scatterplot.zones = data.rows.map(obj => {
                return obj.map_label
            });
    ...
    async function loadTimeSeries(selected) {
        ...
        let data = await $.getJSON(query, () => {
            ...
            dataA = data.rows.filter(obj => {
                return obj.id2 === el.focalUnitA
            })
            dataB = data.rows.filter(obj => {
                return obj.id2 === el.focalUnitB
            })

            el[selected].timeseries.years = [...new Set(data.rows.map(item => item.year))];
            el[selected].timeseries.a = dataA.map(obj => {
                if (obj.id2 == el.focalUnitA) return obj[selection]
            })
            el[selected].timeseries.b = dataB.map(obj => {
                if (obj.id2 == el.focalUnitB) return obj[selection]
            })
